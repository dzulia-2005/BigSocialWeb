...
const Post: React.FC<PropsWithChildren<PostProps>> = ({ post }) => {
  ...
  const likehandler = () => {
    setLike((prevLike) => (islike ? prevLike - 1 : prevLike + 1));
    setIslike(!islike);
    console.log("clicked");
  };
        ...
        <div className='flex items-center gap-[2%]'>
          <NavLink to={`/profile/${post.username}`} >
            <div className='w-[40px] h-[40px] bg-[#D9D9D9] rounded-full'>
              <img className='rounded-full' src={post.img} alt="" />
            </div>
          </NavLink>
          
          <div className='text-[#fff]'>{user?.username || "No username"}</div>
        </div>
        <div>
